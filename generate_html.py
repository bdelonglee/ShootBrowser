...
class HTMLGenerator:
    ...
    def _apply_overlays(self, rows: list) -> None:
        ...
        # 4. Take extras — Shot Names + Parent Take (+ derived Children Takes)
...
function dbGroupKey(row) {{
    switch (dbGroupMode) {{
        ...
        case 'shot_name': {{
            const n = row['_shot_names'] || [];
            return n.length ? n.join(', ') : '(no shot name)';
        }}
...
function dbSortValue(row) {{
    switch (dbSortKey) {{
        ...
        case 'shot_name': {{
            const n = (row['_shot_names'] || []).slice().sort();
            return n.length ? n.join(', ').toLowerCase() : '\\uffff';
        }}
...
function renderDbCard(row, idx) {{
    ...
    const tilt   = row['Tilt']      || '';
    // No margin-left:auto here — .db-title-right already carries it, and having both
    // would split the line's free space between two points, offsetting the chevron
    // from the badges by a gap that varies per card. Let .db-title-right push both
    // the badge cluster and (via the trailing gap) the chevron to the far right.
    const chevron = `<button class="toggle-btn"
