...
def _generate_readme_pdf(buf, project_name, export_date, base_name, bin_names):
    """Render a README PDF explaining the export package into buf."""
    try:
    ...
    sty_title    = _s('RT', fontSize=22, fontName='Helvetica-Bold', textColor=NAVY,
                       leading=26, spaceAfter=1*mm)
    sty_sub      = _s('RS', fontSize=10, textColor=MUTED, spaceAfter=6*mm)
    sty_section  = _s('RH', fontSize=11, fontName='Helvetica-Bold', textColor=NAVY,
                       spaceBefore=9*mm, spaceAfter=3*mm, leading=14)
    sty_filename = _s('RF', fontSize=9,  fontName='Courier-Bold', textColor=BLUE,
                       spaceBefore=5*mm, spaceAfter=1*mm, leading=12)
    sty_body     = _s('RB')
    sty_indent   = _s('RI', leftIndent=5*mm, spaceAfter=2*mm)
    sty_note     = _s('RN', fontSize=8, fontName='Helvetica-Oblique', textColor=MUTED,
                       spaceBefore=14*mm)
    ...
    doc = SimpleDocTemplate(
        ...
        title=f'{project_name} — VFX Export Package',
    )
    story = []

    # ── Header ───────────────────────────────────────────────────────────
    story.append(Paragraph(project_name, sty_title))
    story.append(Paragraph(f'VFX Export Package &mdash; {export_date}', sty_sub))
    story.append(HRFlowable(width='100%', thickness=1, color=NAVY, spaceAfter=6*mm))

    story.append(Paragraph(
        'This package contains data from the VFX shoot database in multiple formats. '
        'The CSV, PDF, and HTML files all contain the same information &mdash; '
        'use whichever suits your workflow.',
        sty_body))

    # ── Package Contents ─────────────────────────────────────────────────
    story.append(Paragraph('Package Contents', sty_section))

    story.append(Paragraph(f'{base_name}.csv', sty_filename))
    story.append(Paragraph(
        'Spreadsheet of all VFX takes with full technical metadata. '
        'Open in Excel, Numbers, or Google Sheets.',
        sty_body))

    story.append(Paragraph(f'{base_name}.pdf', sty_filename))
    story.append(Paragraph(
        'Formatted turnover document with one page per slate. '
        'Includes reference photos, take breakdowns, and production notes.',
        sty_body))

    story.append(Paragraph(f'{base_name}.html', sty_filename))
    story.append(Paragraph(
        'Interactive viewer &mdash; open in any web browser, no internet connection required. '
        'Supports search and filtering by slate, VFX ID, date, and camera.',
        sty_body))

    # ── Bin Files ────────────────────────────────────────────────────────
    if bin_names:
        story.append(Paragraph('Bin Files', sty_section))
        story.append(Paragraph(
            'The following files represent curated selections of takes. '
            'Import them into the HTML viewer to filter the database to that specific subset.',
            sty_body))
        story.append(Spacer(1, 3*mm))
        for name in bin_names:
            story.append(Paragraph(f'&bull;&nbsp;&nbsp;{name}', sty_indent))
        story.append(Spacer(1, 3*mm))
        story.append(Paragraph('<b>To import a bin:</b>', sty_body))
        story.append(Spacer(1, 2*mm))
        steps = [
            f'Open <font name="Courier">{base_name}.html</font> in a web browser.',
            'In the database toolbar, open the Bin selector.',
            'Click <b>Import Bin...</b> at the bottom of the bin list and select the .json file.',
            'The bin is created and activated immediately.',
        ]
        for i, step in enumerate(steps, 1):
            story.append(Paragraph(f'{i}.&nbsp;&nbsp;{step}', sty_indent))

    # ── Footer note ──────────────────────────────────────────────────────
    story.append(Paragraph('Generated by VFX Shoot Browser', sty_note))

    def _footer(canvas, doc_):
        canvas.saveState()
        canvas.setFont('Helvetica', 7)
        canvas.setFillColor(MUTED)
        canvas.drawRightString(A4[0] - 28*mm, 8*mm, f'Page {doc_.page}')
        canvas.restoreState()

    doc.build(story, onFirstPage=_footer, onLaterPages=_footer)


@app.route("/api/export-readme-pdf", methods=["POST"])
def api_export_readme_pdf():
    """Generate and stream a README PDF describing an export package."""
    try:
        body       = request.get_json(force=True) or {}
        base_name  = body.get("base_name", "export")
        bin_names  = body.get("bin_names") or []
        data       = _load_db_json()
        project_name = (data.get("project") or {}).get("name", "VFX Shoot")
        export_date  = datetime.now().strftime('%d/%m/%Y')
        buf = io.BytesIO()
        _generate_readme_pdf(buf, project_name, export_date, base_name, bin_names)
        buf.seek(0)
        return send_file(buf, mimetype="application/pdf",
                         as_attachment=True, download_name="README.pdf")
